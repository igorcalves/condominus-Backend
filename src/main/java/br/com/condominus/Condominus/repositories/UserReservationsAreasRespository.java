...
public interface UserReservationsAreasRespository extends JpaRepository<UserReservationsAreas,String>{
    ...
    @Query(value = "SELECT new br.com.condominus.Condominus.domain.dto.ReservationReturn(ur.area.id,u.name,u.cpf,r.name,ur.startOfScheduling,ur.endOfScheduling) FROM UserReservationsAreas ur INNER JOIN User u ON ur.user.id = u.id INNER  JOIN Areas r ON ur.area.id = r.id WHERE ur.user.id =:id")
